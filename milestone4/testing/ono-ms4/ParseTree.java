...
public class ParseTree {
	...
	public void postTraverse(Nodes root){
		...
		if(current.lchild == null){
			...
			current.primType = current.lchild.primType;
		} else if((current.lchild.primType.equalsIgnoreCase("float") && current.rchild.primType.equalsIgnoreCase("int"))||
				(current.rchild.primType.equalsIgnoreCase("float") && current.lchild.primType.equalsIgnoreCase("int"))){
			current.primType = "float";
			System.out.print("s>f ");
		} else {
			System.err.println("Error encountered in post-traversal of tree.");
			System.err.println("Unexpected: " + current.value.tValue);
			System.exit(-1);
		}
		String prefix = "";
		String postfix = "";
		if(!current.primType.equalsIgnoreCase("float") && current.value.tType.equalsIgnoreCase("realop")){
			System.err.println("Type mismatch");
			System.exit(-1);
		}
		if(current.value.tType.equalsIgnoreCase("expression")){
			if(current.lchild.primType.equalsIgnoreCase("float")){
		...
		} else if(current.primType.equalsIgnoreCase("float")){
